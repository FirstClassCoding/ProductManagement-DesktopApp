...
def addStockList() :
    clearScreen()
    def addList() :
        excel.data.add(name_entry.get() , description_entry.get() , price_entry.get())
    ...
    confirm_button = add.button(text = 'เพิ่มรายการสินค้า' , row = 5 , column = 0 , columnspan = 3 , padx = (0,0) , pady = (10,0) , command = addList)

def showStockList() :
    clearScreen()
    def showList(choice = '') :
        data = excel.data.read().set_index('Name')
        description_text['text'] = data.loc[name_optionMenu['text'] , 'Description']
        price_amount['text'] = data.loc[name_optionMenu['text'] , 'Price']
        remain_amount['text'] = data.loc[name_optionMenu['text'] , 'Amount']
        # description_text['text'] = data.loc[data['Name'] == name_optionMenu['text']]['Description'].to_string(index = False)

    showList_label = add.label(text = 'รายการสินค้าในระบบ' , row = 1 , column = 2 , padx = (0,0) , pady = (0,0))

    options = excel.data.read()['Name']
...
def editStockList() :
    ...
    def showList(choice = '') :
        ...
        description_entry.insert(0 , data.loc[name_optionMenu['text'] , 'Description'])
        price_entry.delete(0 , 'end')
        price_entry.insert(0 , data.loc[name_optionMenu['text'] , 'Price'])
...
def removeStockList() :
    clearScreen()
    def removeList() :
        excel.data.remove(name_optionMenu['text'])
        removeStockList()
...
def buyGoods() :
    clearScreen()
    def addGoods() :
        excel.data.goods(name_optionMenu['text'] , amount_entry.get())
        showList()
